...
ttf_path_chinese = os.path.join(cur_path, "staticData", "test0.ttf")
...
text_folder = os.path.join(cur_path, "staticData", "textData")
cn_zh_path = os.path.join(text_folder, "CNzh.json")
eu_en_path = os.path.join(text_folder, "EUen.json")
brand_traits_path = os.path.join(text_folder, "brandTraits.json")

logger.debug('载入 CNzh.json 翻译文件')
with open(cn_zh_path) as file:
    CN_zh = json.loads(file.read())
    logger.debug('CN_zh 载入成功')
logger.debug('载入 EUen.json 翻译文件')
with open(eu_en_path) as file:
    EU_en = json.loads(file.read())
    logger.debug('EU_en 载入成功')
logger.debug('载入 brand_traits.json 文件')
with open(brand_traits_path) as file:
    brand_traits = json.loads(file.read())
    logger.debug('brand_traits 载入成功')
...
def draw_text_central(back_ground, size, text, color):
    ...
    w, h = ttf.getsize(text)
    text_pos = ((size[0] - w) / 2, (size[1] - h) / 2)
...
def draw_gear_pic(gear_key: str, skill_key_list: list[str], brand_key: str, usual_skill_key: str,
    ...
    # 价格背景图
    price_bg = get_file("news-bg", "jpg").convert('RGBA')
    price_bg_resize = price_bg.resize((back_w - 30, 40), Image.ANTIALIAS)
    ...
    # 粘贴装备名
    price_draw = draw_text_central(price_bg_resize, price_bg_resize.size, gear_name, "white")
