...
def get_gear_image():
    ...
    """
    # 特选商品
    gear_raw_data = deepcopy(get_gear_data())
    pickup_brand_gears_handled = gear_raw_data['gesotown']['pickupBrand']
    pickup_brand_gears_handled['saleEndTime'] = get_gmt_datetime(pickup_brand_gears_handled['saleEndTime'])
    for i in pickup_brand_gears_handled['brandGears']:
        i['saleEndTime'] = get_gmt_datetime(i['saleEndTime'])
    # 普通商品
    limited_gears_handled = gear_raw_data['gesotown']['limitedGears']
    ...
    limited_gears_pics = []
    # 处理特选商店数据
    for pick_up_gear in pickup_brand_gears_handled['brandGears']:
        pickup_gears_pics.append(get_gear_detail(pick_up_gear))

    # 处理普通商店数据
    for gear in limited_gears_handled:
        limited_gears_pics.append(get_gear_detail(gear))
