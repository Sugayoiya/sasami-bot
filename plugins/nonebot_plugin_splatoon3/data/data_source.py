...
def get_gear_data():
    ...
    def check_expire_gear(gear: dict) -> bool:
        """
        检查 gear 是否过期
        :return:
        """
        now = datetime.datetime.now()
        logger.debug('检查 gear_res 文件是否过期')

        sale_end_time_str = gear.get('gesotown', {}).get('pickupBrand', {}).get('saleEndTime', None)
        logger.debug(f'特选商店的 saleEndTime 为 {sale_end_time_str}')
        if not sale_end_time_str:
            logger.debug('特选商店的 saleEndTime 为空，gear.json 文件已过期')
            return True
        limited_gears_list = gear.get('gesotown', {}).get('limitedGears', [])
        if not limited_gears_list:
            logger.debug('限定商店的 limitedGears 为空，gear.json 文件已过期')
            return True
        # 如果有一个限定品的结束时间小于当前时间，就返回False
        for i in limited_gears_list:
            limited_sale_end_time = i.get('saleEndTime', None)
            logger.debug(f'普通商店的{i.get("id", None)} saleEndTime 为 {limited_sale_end_time}')
            if not limited_sale_end_time or get_gmt_datetime(limited_sale_end_time) < now:
                logger.debug(
                    f'普通商店的{i.get("id", None)} saleEndTime 为空或 {limited_sale_end_time} 已过期，gear.json 文件已过期')
                return True
        # 如果精选商品的结束时间小于当前时间，就返回False
        if get_gmt_datetime(sale_end_time_str) < now:
            logger.debug(f'特选商店的 saleEndTime {sale_end_time_str} 已过期，gear.json 文件已过期')
            return True
        return False

    if gear_res is None or check_expire_gear(gear_res):
        logger.debug('gear不存在或已过期，重新获取')
        logger.info("重新请求:装备数据")
        result = cf_http_get("https://splatoon3.ink/data/gear.json").text
        gear_res = json.loads(result)
        gear_res = gear_res['data']
        return gear_res
    else:
        return gear_res
