...
# 打工 触发器
matcher_coop = on_regex("^[\\\/\.。]?(全部)?(工|打工|鲑鱼跑|bigrun|big run|团队打工)$", priority=10, block=True,
                        rule=msg_rule)
...
# 其他命令 触发器处理
@matcher_else.handle()
async def _(matcher: Matcher, bot: Bot, event: EventV11):
    plain_text = event.get_message().extract_plain_text().strip()
    # 触发关键词  同义文本替换
    plain_text = multiple_replace(plain_text, dict_keyword_replace)
    logger.info("同义文本替换后触发词为:" + plain_text + "\n")
    ...
    if re.search("^随机武器.*$", plain_text):
        ...
        if isinstance(event, GroupMessageEvent):
            pickup_gears_pics, limited_gears_pics = get_gear_image()

            rst = [SegmentV11.text("特选商品为:")]
            for i in pickup_gears_pics:
                rst.append(SegmentV11.image(image_to_base64(i), cache=False))
            rst.append(SegmentV11.text("通常商品为:"))
            for i in limited_gears_pics:
                rst.append(SegmentV11.image(image_to_base64(i), cache=False))
...
matcher_admin = on_regex("^[\\\/\.。]?(重载武器数据|更新武器数据|清空图片缓存)$", priority=10, block=True,
