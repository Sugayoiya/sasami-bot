...
group_add = on_command("添加菜单", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=15, block=True)
...
add_greeting = on_command("添加问候", aliases={"添加问候语"}, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER,
                          priority=12,
                          block=True)
remove_greeting = on_command("删除问候", aliases={"删除问候语", "移除问候", "移除问候语"},
                             permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=12, block=True)
...
# 午餐提醒
@scheduler.scheduled_job("cron", hour=10, minute=45, misfire_grace_time=60)
...
# 晚餐提醒
@scheduler.scheduled_job("cron", hour=17, minute=30, misfire_grace_time=60)
...
# 夜宵提醒
@scheduler.scheduled_job("cron", hour=21, minute=30, misfire_grace_time=60)
