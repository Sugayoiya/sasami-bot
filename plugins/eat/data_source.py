...
class EatingManager:
    def __init__(self):
        ...
        self._quote_json: Path = what2eat_config.what2eat_path / "quote.json"
    ...
    def get_percentage_eat_str(self, item: List[str]) -> str:
        '''
            获取百分比菜单
        '''
        select_food_list = random.choices(item, k=what2eat_config.eating_sample_count)
        percent = [random.randint(0, 100) for _ in range(what2eat_config.eating_sample_count)]
        ...
        self._quote = load_json(self._quote_json)
        return "\n".join(choice_list)
    ...
    def get2eat(self, event: MessageEvent) -> Tuple[Message, MessageSegment]:
        ...
        if self._eating["count"][gid][uid] >= what2eat_config.eating_limit:
            ...
            select_food_list = random.choices(food_list, k=what2eat_config.eating_sample_count)
            percent = [random.randint(0, 100) for _ in range(what2eat_config.eating_sample_count)]
            percent_int = [int(i / sum(percent) * 100) for i in percent[:-1]]
            percent_int.append(100 - sum(percent_int))
            choice_list = []
            ...
            msg = MessageSegment.text("建议 ") + Message(self.get_percentage_eat_str(food_list))
    ...
    def get2drink(self, event: MessageEvent) -> MessageSegment:
        ...
        if self._eating["count"][gid][uid] >= what2eat_config.eating_limit:
            ...
            return MessageSegment.text(self.get_percentage_drinks_str())
            # return MessageSegment.text(random.choice(
            #     [
            #         f"不如来杯 {_branch} 的 {_drink} 吧！",
            #         f"去 {_branch} 整杯 {_drink} 吧！",
    ...
    def get_percentage_drinks_str(self) -> str:
        ...
        _drinks: Dict[str, List[str]] = load_json(self._drinks_json)
        select_drink_list = []
        for i in range(what2eat_config.eating_sample_count):
            _branch: str = random.choice(list(_drinks))
            _drink: str = random.choice(_drinks[_branch])
