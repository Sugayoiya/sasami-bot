...
@roll.handle()
async def handle_first_receive(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg(),
...
async def handle_roll(bot: Bot, event: GroupMessageEvent, state: T_State = T_State):
    _roll = str(state['rd'])
    log.info(f'roll: {_roll}, type of roll: {type(_roll)}')
